...
public class Executors {
    ...
    public static Executor create(Class<? extends Executor> superClass) {
        return ProxyFactory.proxy().newProxyInstance(superClass, new MessageIntercept());
    }

    public static Executor create(Class<? extends Executor> superClass, Object... args) {
        return ProxyFactory.proxy().newProxyInstance(superClass, new MessageIntercept(), args);
    }
     ...
     * 2.进行对应的方法调用
     */
    static class MessageIntercept implements CglibInvocationInterceptor {
        ...
        public Object invoke(Object object, Method method, Object[] args, MethodProxy proxy) throws Throwable {
            ...
            StorageProperties storageProperties = ((Executor) object).getStorageProperties();
            ...
            if (read) {
                recoder = chooser(((Executor) object).getReadRecords(), args);
            } else if (write) {
                recoder = chooser(((Executor) object).getWriteRecords(), args);
            }
            if (recoder != null) {
